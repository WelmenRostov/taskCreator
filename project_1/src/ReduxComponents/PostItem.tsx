...
import { saveEditor, activEditor, activFulfielled, activRejected } from './features/todos/todoThunk';
...
const PostItem = ({
  ...
  const handleToFulfielled = () => {
    dispatch(activFulfielled({ id }));
  };

  const handleToRejected = () => {
    dispatch(activRejected({ id }));
  };

  if (conditionTasks == 'rejected') {
    return (
      <>
        <motion.div
          layout
        ...
        >
          ...
          >
            ...
            <div className="grid grid-flow-row-dense grid-cols-1 grid-rows-1 justify-items-end mt-3">
              {/*
              <MyButton className="btn btn-soft w-[130px] m-2 col-end-4 row-end-1">Восстановить</MyButton>*/}
  ...
  if (conditionTasks == 'fulfilled') {
    return (
      <>
        ...
        >
          <div
            className={`rounded-[1vw] mb-5 p-5 outline-black/5 bg-opacity-10 overflow-hidden resize-none bg-green-700/50 shadow-lg shadow-green-700/90 border-2 border-green-600`}
          >
            <h1 className="break-words text-[30px] flex items-start">
              {order}.
              {editable ? (
  ...
  if (conditionTasks == 'pending') {
    return (
      <>
        ...
        >
          <div
            className={`rounded-[1vw] mb-5 p-5 outline-black/5 bg-opacity-10 overflow-hidden resize-none bg-yellow-600/30 ${
              editable
          ...
          >
            ...
            {editable ? (
              ...
              <p className="min-h-[50px] w-full pt-2 overflow-x-auto resize-none flex-grow break-words">
                {initialText}
              </p>
            )}
            ...
            <div className="grid grid-flow-row-dense grid-cols-1 grid-rows-1 justify-items-end mt-3">
              {editable ? (
                ...
                <>
                  <MyButton className="btn btn-soft m-2 col-end-1 row" onClick={handleToFulfielled}>
                    Выполнена
                  </MyButton>
                  <MyButton className="btn btn-soft m-2 -mr-[40px] col-end-3" onClick={handleEdit}>
                    Изменить
                  </MyButton>
                  <MyButton className="btn btn-soft w-[100px] m-2 col-end-4 row-end-1" onClick={handleToRejected}>
                    Удалить
                  </MyButton>
                </>
              )}
